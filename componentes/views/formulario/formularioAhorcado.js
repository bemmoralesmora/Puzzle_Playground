...
export function cargarFormularioAhorcado() {
  let formulario = document.createElement("div");
  formulario.className = "formulario-ahorcado";
  ...
  nombre.placeholder = "Nombre del juego de ahorcado";
  ...
  jugar.textContent = "Jugar al Ahorcado";
  jugar.className = "btn-crear-partida";
  jugar.addEventListener("click", async () => {
    verificarCamposYGenerarCodigo(); // ✅ Esto fuerza que el código se actualice antes de validarlo
    ...
    try {
      ...
      console.log("✅ Partida de ahorcado guardada:", resultado);
      ...
      DOM.querySelector(".btn-comenzar-partida").addEventListener(
        ...
        async () => {
          ...
          try {
            console.log("🚀 Click en comenzar juego de ahorcado");
            const socket = await conectarSocket(idLogin, token);
            ...
            socket.once("actualizar_jugadores", () => {
              console.log(
                "✅ Jugador unido. Enviando comenzar_partida_ahorcado..."
              );
              socket.emit("comenzar_partida", { idPartida, idLogin });
            });

            socket.once("partida_comenzada", (data) => {
              console.log("🎮 Partida de ahorcado comenzó:", data);
              DOM.innerHTML = "";
              DOM.appendChild(
                cargarAhorcado(parseInt(data.nivel), data.dificultad)
              );
            });

            socket.once("error_partida", (error) => {
              console.error(
                "❌ Error al unirse a partida de ahorcado:",
                error.mensaje
              );
      ...
      console.error("Error al guardar partida de ahorcado:", error);
      const DOM = document.querySelector("#root");
      DOM.innerHTML = "";
      ...
      errorDiv.innerHTML = `
        <h3>Error al guardar partida de ahorcado</h3>
        <p>${error.message}</p>
      ...
      DOM.querySelector(".btn-reintentar").addEventListener("click", () => {
        DOM.innerHTML = "";
        DOM.appendChild(cargarFormularioAhorcado());
      });
    }
  });
