    ...
    try {
      ...
      DOM.querySelector(".btn-comenzar-partida").addEventListener(
        ...
        async () => {
          ...
          try {
            console.log("🚀 Click en comenzar juego de memoria");
            const socket = await conectarSocket(idLogin, token);
            ...
            socket.once("actualizar_jugadores", () => {
              console.log(
                "✅ Jugador unido. Enviando comenzar_partida_memoria..."
              );
              socket.emit("comenzar_partida", { idPartida, idLogin });
            });

            socket.once("partida_comenzada", (data) => {
              console.log("🎮 Partida de memoria comenzó:", data);
              DOM.innerHTML = "";
            ...
            socket.once("error_partida", (error) => {
              console.error(
                "❌ Error al unirse a partida de memoria:",
                error.mensaje
              );
      ...
      console.error("Error al guardar partida de memoria:", error);
      const DOM = document.querySelector("#root");
      DOM.innerHTML = "";
      ...
      errorDiv.innerHTML = `
        <h3>Error al guardar partida de memoria</h3>
        <p>${error.message}</p>
      ...
      DOM.querySelector(".btn-reintentar").addEventListener("click", () => {
        DOM.innerHTML = "";
        DOM.appendChild(cargarFormularioMemoria());
      });
    }
  });
